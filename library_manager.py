...
def create_visualations(stats):
    ...
    if stats['decades']:
        decades_df = pd.DataFrame({
            ...
            'Count': list(stats['decades'].value())
        })
        fig_decades = px.line(
            decades_df,
            x='Decade',
...
lottie_book =load_lottieurl("https://assests9.lottieflies.com/temp/1f20_acAfIn.json")
if lottie_book:
    with st.sidebar:
...
""", unsafe_allow_html=True)
    ...
    if not st.session_state.library:
        ...
        if stats['authors']:
            ...
            top_authors = dict(list(stats['authors'].item())[:5])
            for author, count in top_authors.items():
                st.markdown(f"**{author}**: {count} book{'s' if count > 1 else ""}")
st.markdown("===")
st.markdown("Copyright @ 2025 Areej Shah Personal Library Manager", unsafe_allow_html=True)
